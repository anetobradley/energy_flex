...
IO <- function(samples, designParam, posteriors, weather){
  ...
  yc[i] = (sum(PredictedUsage))/(sum(floorArea)); #average energy use per m^2 for dwellings for given sample of inputs
...
standriver = function(yf,yc,xf,xc,tc){
  ...
  eta_mu <- mean(yc, na.rm = TRUE) # mean value
  eta_sd <- sd(yc, na.rm = TRUE) # standard deviation
  yf <- (yf - eta_mu) / eta_sd
  yc <- (yc - eta_mu) / eta_sd
